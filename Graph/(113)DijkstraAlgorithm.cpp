...
{
    set<pair<int, int>> st;
    vector<int> dist(V, INT_MAX); // answer vector
    st.insert({0, s});
    dist[s] = 0;
    while (st.size())
    {
        ...
        for (auto nbr : adj[node])
        {
            int weight = nbr[1];
            int nodeV = nbr[0];
            if (distance + weight < dist[nodeV])
            {
                if (st.find({dist[nodeV], nodeV}) != st.end())
                    st.erase(st.find({dist[nodeV], nodeV}));

                dist[nodeV] = distance + weight;
                st.insert({dist[nodeV], nodeV});
            }
        }
    }
    return dist;

...
Level: Hard 
Companies: DoorDash | Amazon | Facebook | Microsoft | Google | ByteDance | tiktok | Oracle | Snapchat | Bloomberg | Adobe | Twitter | Twilio | Sprinklr | TuSimple
...
int mxPathSum(TreeNode *root, int &mx)
{
    ...
    int lSum = max(0, mxPathSum(root->left, mx));
    int rSum = max(0, mxPathSum(root->right, mx));

    int subtreeSum = root->val + lSum + rSum;
    int pathSum = root->val + max(lSum, rSum);

    mx = max(mx, subtreeSum);
